...
class RecordConfig:
    def __init__(self, cfg: Dict[str, Any]):
        ...
        self.camera_backend: str = cam.get("backend", "realsense")
        self.pybullet_urdf_path: str = cam.get("pybullet_urdf_path", "")
...
def make_camera_configs(record_cfg: RecordConfig) -> dict:
    ...
    if record_cfg.camera_backend == "pybullet":
        from lerobot_robot_ur5e import PyBulletCameraConfig

        if not record_cfg.pybullet_urdf_path:
            raise ValueError(
                "cameras.pybullet_urdf_path is required when backend is 'pybullet'."
            )

        logging.info(
            "====== [CAM] Using PyBullet dual rendered views for %s "
            "(both are external synthetic viewpoints of the robot's true pose, "
            "NOT first-person views from a mounted camera) ======",
            ", ".join(camera_names),
        )
        return {
            # Front-ish view
            "wrist_image": PyBulletCameraConfig(
                robot_ip=record_cfg.robot_ip,
                urdf_path=record_cfg.pybullet_urdf_path,
                fps=record_cfg.fps,
                width=record_cfg.width,
                height=record_cfg.height,
                camera_eye=(1.2, 1.2, 1.0),
                camera_target=(0, 0, 0.3),
            ),
            # Side/higher angle view - genuinely different viewpoint
            "exterior_image": PyBulletCameraConfig(
                robot_ip=record_cfg.robot_ip,
                urdf_path=record_cfg.pybullet_urdf_path,
                fps=record_cfg.fps,
                width=record_cfg.width,
                height=record_cfg.height,
                camera_eye=(-1.0, 1.4, 1.6),
                camera_target=(0, 0, 0.2),
            ),
        }

    if record_cfg.camera_backend == "opencv":
        from lerobot.cameras.configs import ColorMode, Cv2Rotation
        from lerobot.cameras.opencv.configuration_opencv import OpenCVCameraConfig
        from lerobot_robot_ur5e import PyBulletCameraConfig

        if not record_cfg.exterior_cam_serial:
            raise ValueError(
                "exterior_cam_serial (OpenCV device index, e.g. '0') is required "
                "when cameras.enabled is true and backend is 'opencv'."
            )
        if not record_cfg.pybullet_urdf_path:
            raise ValueError(
                "cameras.pybullet_urdf_path is required when cameras.enabled is "
                "true and backend is 'opencv' (used to render wrist_image)."
            )

        logging.info(
            "====== [CAM] Using OpenCV backend for exterior_image (real webcam); "
            "wrist_image rendered live via PyBullet ======"
        )
        return {
            "wrist_image": PyBulletCameraConfig(
                robot_ip=record_cfg.robot_ip,
            ...
            "exterior_image": OpenCVCameraConfig(
                index_or_path=int(record_cfg.exterior_cam_serial),
                fps=record_cfg.fps,
                width=record_cfg.width,
                height=record_cfg.height,
